...
3. **The client must stay compatible with the network's upgrade schedule.** This one was missing from the list and is the sharpest of the three, because it is not under the chain's control at all and it runs on somebody else's calendar. `avalanchego v1.15.0-fuji` shipped on 2026-07-21 with `MinimumCompatibleVersion = 1.15.0` and a hard deadline — *"all Fuji nodes must upgrade before 11 AM ET, July 28th 2026"*. CSB's nodes were on `v1.14.1`. From that moment Fuji peers **refused the handshake**, so every Primary Network validator appeared in the P-Chain health check's `disconnectedValidators` while the node still reported 3–4 peers, the P-Chain never finished bootstrapping, and **CSB could not start**, because a chain cannot learn its own validator set without the P-Chain. The outage began on 28 July and was diagnosed on 2 August; nothing raised a flag in between.
